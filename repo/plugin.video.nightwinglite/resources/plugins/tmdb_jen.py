...
def run(url,lang,icon,fanart,plot,name):
    ...
    if 'year/movie' in url:
        url2=f'https://api.themoviedb.org/3/discover/movie?api_key=60fe3606be5342a8143b5428bc9e9d6e&language=%s&sort_by=popularity.desc&include_adult=false&include_video=false&primary_release_year=%s&with_original_language=en&page=1'%(lang,url.split('year/movie/')[1])
        mode=14
        ...
        url2=f'https://api.themoviedb.org/3/discover/movie?api_key=60fe3606be5342a8143b5428bc9e9d6e&with_companies={company_id}&language={lang}&sort_by=popularity.desc&timezone=America%2FNew_York&include_null_first_air_dates=false&page=1'
        mode=14
    elif 'keyword' in url:
        split_url = url.split("/")
        if len(split_url) == 3:
        ...
        url2=f'https://api.themoviedb.org/3/discover/%s?with_keywords=%s&api_key=60fe3606be5342a8143b5428bc9e9d6e&language=%s&page=1'%(s_type,keyword_id,lang)
        mode=14
    elif 'collection/' in url:
        split_url = url.split("/")
        id = split_url[-1]
        url2=f'https://api.themoviedb.org/3/%s?api_key=60fe3606be5342a8143b5428bc9e9d6e&language=%s&page=1'%(url,lang)
        mode=179
    elif 'network' in url:
        split_url = url.split("/")
        if len(split_url) == 3:
        ...
        url2=f'https://api.themoviedb.org/3/discover/tv?api_key=60fe3606be5342a8143b5428bc9e9d6e&with_networks={network_id}&language={lang}&sort_by=popularity.desc&timezone=America%2FNew_York&include_null_first_air_dates=false&page=1'
        mode=14
    elif 'genre' in url:
        split_url = url.split("/")
        if len(split_url) == 3:
        ...
        if media == "movie":
            ...
            s_type='tv'
        url2=f'https://api.themoviedb.org/3/discover/%s?api_key=60fe3606be5342a8143b5428bc9e9d6e&sort_by=popularity.desc&with_genres=%s&language=%s&page=1'%(s_type,genre_id,lang)
        mode=14
    elif 'list/' in url:
        id = url.split("/")[-1]
        url2=f'https://api.themoviedb.org/3/%s?api_key=60fe3606be5342a8143b5428bc9e9d6e&language=%s&page=1'%(url,lang)
