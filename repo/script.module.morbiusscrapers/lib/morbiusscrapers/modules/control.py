...
addonObject = addon('script.module.morbiusscrapers')
...
def setting(id, fallback=None):
	...
	try:
		cached_mtime = homeWindow.getProperty('morbiusscrapers_settings_mtime')
		...
		if current_mtime and current_mtime != cached_mtime:
			...
			settings_dict = jsloads(homeWindow.getProperty('morbiusscrapers_settings'))
...
def make_settings_dict(): # service runs upon a setting change
	try:
		...
		homeWindow.setProperty('morbiusscrapers_settings', jsdumps(settings_dict))
		try: homeWindow.setProperty('morbiusscrapers_settings_mtime', str(os.path.getmtime(settingsFile)))
		except Exception: pass
		return settings_dict
	except:
		return None

def refresh_debugReversed(): # called from service "onSettingsChanged" to clear morbiusscrapers.log if setting to reverse has been changed
	if homeWindow.getProperty('morbiusscrapers.debug.reversed') != setting('debug.reversed'):
		homeWindow.setProperty('morbiusscrapers.debug.reversed', setting('debug.reversed'))
		execute('RunPlugin(plugin://script.module.morbiusscrapers/?action=tools_clearLogFile)')
...
def isVersionUpdate():
	...
	try:
		...
		xbmc.log('morbiusscrapers Addon Data Path Does not Exist. Creating Folder....', LOGINFO)
		addon_folder = transPath('special://profile/addon_data/script.module.morbiusscrapers')
		xbmcvfs.mkdirs(addon_folder)
	try:
		with open(versionFile, 'r') as fh: oldVersion = fh.read()
	except: oldVersion = '0'
	try:
		curVersion = addon('script.module.morbiusscrapers').getAddonInfo('version')
...
def checkPackages():
	...
	for item in files:
		if '.morbiusscrapers' in item and item.endswith('zip'):
...
def clean_settings():
	def _make_content(dict_object):
		...
		content += '\n</settings>'
		return content
	addon_id = 'script.module.morbiusscrapers'
	try:
		removed_settings = []
		active_settings = []
