...
import xbmc
from morbiusscrapers.modules import control, log_utils
...
class CheckSettingsFile:
	def run(self):
		try:
			xbmc.log('[ script.module.morbiusscrapers ]  CheckSettingsFile Service Starting...', LOGINFO)
			window.clearProperty('morbiusscrapers')
			...
			if not control.existsPath(settings_xml):
				control.setSetting('module.provider', 'morbiusscrapers')
				log_utils.log('%s : created successfully' % settings_xml, LOGINFO)
			else: log_utils.log('%s : already exists' % settings_xml, LOGINFO)
			return xbmc.log('[ script.module.morbiusscrapers ]  Finished CheckSettingsFile Service', LOGINFO)
		except:
			import traceback
			traceback.print_exc()
...
class SettingsMonitor(control.monitor_class):
	def __init__ (self):
		...
		window.setProperty('morbiusscrapers.debug.reversed', control.setting('debug.reversed'))
		xbmc.log('[ script.module.morbiusscrapers ]  Settings Monitor Service Starting...', LOGINFO)
...
class CheckUndesirablesDatabase:
	def run(self):
		xbmc.log('[ script.module.morbiusscrapers ]  "CheckUndesirablesDatabase" Service Starting...', LOGINFO)
		from morbiusscrapers.modules import undesirables
		try:
			...
			import traceback
			traceback.print_exc()
		return xbmc.log('[ script.module.morbiusscrapers ]  Finished "CheckUndesirablesDatabase" Service', LOGINFO)

def main():
	while not control.monitor.abortRequested():
		xbmc.log('[ script.module.morbiusscrapers ]  Service Started', LOGINFO)
		CheckSettingsFile().run()
		CheckUndesirablesDatabase().run()
		if control.isVersionUpdate():
			control.clean_settings()
			xbmc.log('[ script.module.morbiusscrapers ]  Settings file cleaned complete', LOGINFO)
		break
	SettingsMonitor().waitForAbort()
	xbmc.log('[ script.module.morbiusscrapers ]  Service Stopped', LOGINFO)
