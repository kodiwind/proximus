...
def sys_exit_check():
        if get_property('morbius.reuse_language_invoker') == 'false': return False
        return external()
...
def routing(sys):
        ...
        if 'navigator.' in mode:
                ...
                if mode == 'playback.meta':
                        ...
                        morbiusPlayer().run(_get('url', None), _get('obj', None))
        elif 'choice' in mode:
        ...
        elif mode == 'clear_morbius_log':
                from modules.kodi_utils import clear_morbius_log
                clear_morbius_log()
