...
from modules import metadata, settings, tracking
...
def get_hidden_progress_items(watched_indicators):
	try:
		if watched_indicators == 1:
			...
			return trakt_get_hidden_items('dropped')
		# 0 (built-in) and 2 (Simkl): local JSON list in the active watched DB
...
def update_hidden_progress(media_id):
	...
	if watched_indicators == 1:
		from apis.trakt_api import hide_unhide_progress_items as function
...
def hide_unhide_progress_items(params):
	...
	# Always persist locally against the active DB (built-in or Simkl). Trakt hide is separate.
	if indicators == 1: indicators = 0
	current_items = get_hidden_progress_items(indicators) or []
	if action == 'drop': current_items.append(media_id)
	else:
		try: current_items.remove(media_id)
		except ValueError: pass
	watched_db = get_database(indicators)
	watched_db.execute('INSERT OR REPLACE INTO watched_status VALUES (?, ?, ?)', ('hidden_progress_items', 'hidden', json.dumps(current_items),))
...
def get_bookmarks_episode(media_id, season, watched_db=None):
	...
	try:
		...
		# Normalize episode keys to int so lookups match meta episode numbers.
		info = dict([(int(i[3]), {'resume_point': i[0], 'curr_time': i[1], 'resume_id': i[2]}) for i in info])
	except: info = {}
	return info
...
def get_bookmarks_all_episode(media_id, total_seasons, watched_db=None):
	...
	try:
		rows = watched_db.execute(
			...
			('episode', str(media_id))).fetchall()
		for row in rows:
			all_seasons_info.setdefault(int(row[0]), {})[int(row[1])] = {'resume_point': row[2], 'curr_time': row[3], 'resume_id': row[4]}
	except: pass
	return all_seasons_info

def get_progress_status_episode(progress_info, episode):
	try: percent = str(round(float(progress_info[int(episode)]['resume_point'])))
	except:
...
def get_progress_status_all_episode(progress_info, season, episode):
	try: percent = str(round(float(progress_info[int(season)][int(episode)]['resume_point'])))
	except:
...
def get_resume_seconds(progress, duration):
	try: duration = float(duration or 0)
	except: duration = 0
	# Zero/missing runtime (common on older TMDb shows) would yield a 0s resume point —
	# Kodi then draws no resume arrow. Fall back to a 30-minute runtime.
	if duration <= 0: duration = 30 * 60
	return float(int(float(progress)/100 * duration)), duration

def apply_resume_to_listitem(listitem, info_tag, progress, duration):
	"""
	Bind in-progress state so skins can draw the resume arrow / percent bar.
	Uses InfoTagVideo.setResumePoint(seconds, total) only — do not set the
	deprecated ListItem ResumeTime/TotalTime properties (Kodi warns on those).
	Returns True if progress was applied.
	"""
	if not progress: return False
	resume_secs, total_secs = get_resume_seconds(progress, duration)
	if resume_secs <= 0 or total_secs <= 0: return False
	try: info_tag.setDuration(int(total_secs))
	except: pass
	try: info_tag.setResumePoint(float(resume_secs), float(total_secs))
	except:
		try: info_tag.setResumePoint(float(resume_secs))
		except: return False
	try:
		# Aeon / Crew convention: overlay 6 = unwatched-with-resume cue
		if hasattr(info_tag, 'setOverlay'): info_tag.setOverlay(6)
	except: pass
	try:
		...
		try:
			listitem.setProperty('WatchedProgress', str(progress))
...
def clear_local_bookmarks():
	try:
		...
		file_ids = dbcon.execute("SELECT idFile FROM files WHERE strFilename LIKE 'plugin.video.morbius%'").fetchall()
	...
	try:
		...
		if watched_indicators in (1, 2):
			try:
				...
				sleep(1000)
				tracking.progress('clear_progress', media_type, media_id, 0, season, episode, resume_id)
			except: pass
			...
			def _process():
				for i in insert_list:
					try:
						...
						sleep(1000)
						tracking.progress('clear_progress', i[0], i[1], 0, i[2], i[3], resume_id)
					except: pass
