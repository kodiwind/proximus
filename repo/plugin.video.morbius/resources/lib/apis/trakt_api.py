...
def call_trakt(path, params={}, data=None, is_delete=False, with_auth=True, method=None, pagination=False, page_no=1):
	...
	def send_query():
		resp = None
		if with_auth:
			while kodi_utils.get_property('morbius.trakt_refreshing_token') == 'true':
				set_at = float(kodi_utils.get_property('morbius.trakt_refreshing_token_at') or '0')
			...
			try: expires_at = float(get_setting('morbius.trakt.expires'))
			...
			# Skip entirely when there is no real account/token (revoked state uses token='0').
			_tok = get_setting('morbius.trakt.token')
			if _tok and _tok not in (None, '', '0', 'empty_setting') and time.time() > (expires_at - 7*24*3600):
				trakt_refresh_token()
			token = get_setting('morbius.trakt.token')
			# Revoke stores token as '0' — that is truthy in Python and must NOT be sent
			# as Authorization (Trakt returns 403 Forbidden on public+private endpoints).
			if token and token not in (None, '', '0', 'empty_setting'):
	...
	if status_code == 401:
		if with_auth and settings.trakt_user_active():
			...
			if time.time() > (expires_at - 3600): trakt_refresh_token()
...
def trakt_get_device_code():
	...
	if not isinstance(result, dict) or not result.get('device_code'):
		kodi_utils.logger('Trakt Auth', 'oauth/device/code failed — check Client ID/Secret (Trakt app at trakt.tv/oauth/applications)')
		kodi_utils.notification('Trakt Client ID rejected (401/403). Re-enter API keys.', 5000)
		return None
	return result
...
def trakt_get_device_token(device_codes):
	...
	if not isinstance(device_codes, dict) or not device_codes.get('device_code'):
		return None
	result = None
	try:
...
def trakt_refresh_token():
	...
	# Previous behaviour: the `morbius.trakt_refreshing_token` flag was set INSIDE this function
	...
	try:
		...
		for _ in range(40):
			if kodi_utils.get_property('morbius.trakt_refreshing_token') != 'true': break
			set_at = float(kodi_utils.get_property('morbius.trakt_refreshing_token_at') or '0')
			if time.time() - set_at > 30: break  # stale lock — previous holder crashed
			kodi_utils.sleep(250)
		...
		try: last_failed_at = float(get_setting('morbius.trakt.last_refresh_failed_at', '0'))
		except: last_failed_at = 0.0
		if time.time() < last_failed_at + 60:
			return
		kodi_utils.set_property('morbius.trakt_refreshing_token', 'true')
		kodi_utils.set_property('morbius.trakt_refreshing_token_at', str(time.time()))
		current_refresh = get_setting('morbius.trakt.refresh')
		...
		if isinstance(response, dict) and response.get('access_token') and response.get('refresh_token'):
			...
			kodi_utils.logger('morbius', 'Trakt refresh did not return new tokens; backing off 60s')
	except Exception as e:
		try: set_setting('trakt.last_refresh_failed_at', str(time.time()))
		except: pass
		kodi_utils.logger('morbius', 'Trakt refresh exception: %s' % str(e))
	finally:
		kodi_utils.clear_property('morbius.trakt_refreshing_token')
		kodi_utils.clear_property('morbius.trakt_refreshing_token_at')
...
def trakt_authenticate(dummy=''):
	...
	if token:
		...
		# Only claim XOR if no other remote tracker is already active (keep Simkl soak intact).
		if not settings.simkl_user_active():
		...
		# Full watched sync only when Trakt owns XOR; otherwise just confirm auth for list browse.
...
def trakt_revoke_authentication(dummy=''):
	current_token = get_setting('morbius.trakt.token')
...
def trakt_fetch_collection_watchlist(list_type, media_type):
	# FIX (collections capped at 100): get_trakt() with pagination=True only ever returns
	# page 1, and Trakt now enforces pagination on the sync endpoints, so page 1 is capped
	# at ~100 items. Route through get_trakt_all_pages() (same helper the watched
	# indicators use) so every page is fetched and merged.
	def _process(params):
		data = get_trakt_all_pages(params) or []
	...
	params = {'path': path, 'path_insert': (list_type, url_type), 'with_auth': True}
	return trakt_cache.cache_trakt_object(_process, string, params)
...
def trakt_calendar_days(recently_aired, current_date):
	if recently_aired: start, finish = (current_date - timedelta(days=14)).strftime('%Y-%m-%d'), '14'
		...
		previous_days = int(get_setting('morbius.trakt.calendar_previous_days', '0'))
		future_days = int(get_setting('morbius.trakt.calendar_future_days', '7'))
...
def trakt_sync_activities(force_update=False):
	...
	def refresh_token_check():
		...
		if current_time >= (expires_at - 3600): return True
	...
	def _check_daily_expiry():
		return int(time.time()) >= int(get_setting('morbius.trakt.next_daily_clear', '0'))
