...
def addon_version():
	try: return get_setting('morbius.addon_version', '1.0.0')
	except: return '1.0.0'
		...
		if status_code == 401:
			if with_auth and xbmc_player().isPlaying() is False:
				if confirm_dialog(heading='Authorize Simkl', text='You must authenticate with Simkl. Do you want to authenticate now?') and simkl_authenticate():
					headers['Authorization'] = 'Bearer ' + get_setting('simkl.token')
					response = send_query()
					if response is None: return None
					status_code = response.status_code
				else: return None
...
def simkl_get_pin_token(pin_info):
	...
	try:
		...
		from modules.utils import make_qrcode
		qr_icon = make_qrcode(PIN_PAGE % user_code)
		tiny_url = PIN_PAGE % user_code
...
def simkl_authenticate(dummy=''):
	...
	if token and token.get('access_token'):
		...
		# XOR: offer to make Simkl the active watched-status provider (value 2).
		if settings.watched_indicators() != 2:
			if (not settings.trakt_user_active()) or confirm_dialog(heading='Active Tracker',
					text='Make Simkl your active watched-status provider now?[CR]Choose No to keep your current provider.'):
		...
		kodi_utils.set_property('morbius.simkl.user', get_setting('simkl.user') or get_setting('morbius.simkl.user'))
...
def simkl_revoke_authentication(dummy=''):
	...
	if settings.watched_indicators() == 2:
		if settings.trakt_user_active() and confirm_dialog(heading='Active Tracker',
				text='Switch watched-status provider to Trakt?[CR]Choose No to use built-in tracking.'):
			set_setting('watched_indicators', '1')
		else: set_setting('watched_indicators', '0')
	kodi_utils.set_property('morbius.simkl.user', 'empty_setting')
...
def _echo_pause(result, media, media_id, percent, season=None, episode=None, title=''):
	# Always write a local progress row. Simkl pause responses often omit `id` /
	# progress fields; waiting on sync/activities is also wrong because the
	# background throttle frequently returns "not needed" after a pause.
	resume_id, pct, echo_title = 0, percent, title or ''
	if isinstance(result, dict):
		resume_id = result.get('id') or 0
		node = result.get('movie') or result.get('show') or result.get('anime') or {}
		if not echo_title: echo_title = node.get('title', '') or ''
		try: pct = round(float(result.get('progress', percent)), 1)
		except: pct = percent
	last_played = time.strftime('%Y-%m-%dT%H:%M:%S.000Z', time.gmtime())
	if media in ('movie', 'movies'):
		simkl_watched_cache.upsert_progress(('movie', str(media_id), '', '', str(pct), 0, last_played, resume_id, echo_title))
	else:
		ep = (result.get('episode') if isinstance(result, dict) else None) or {}
		try:
			s, n = int(ep.get('season', season)), int(ep.get('number', episode))
		except:
			try: s, n = int(season), int(episode)
			except: return
		simkl_watched_cache.upsert_progress(('episode', str(media_id), s, n, str(pct), 0, last_played, resume_id, echo_title))

def simkl_progress(action, media, media_id, percent, season=None, episode=None, resume_id=None, refresh_tracker=False, title=''):
	# clear_progress: best-effort remote delete when we have a resume_id; local row is erased by watched_status.
	if action == 'clear_progress':
		if resume_id not in (None, '', 0, '0'):
			try: call_simkl('sync/playback/%s' % resume_id, is_delete=True)
			except: pass
		return
	result = None
	try: result = simkl_scrobble('pause', media, media_id, percent, season, episode)
	except Exception as e:
		logger('Simkl scrobble pause', str(e))
		result = None
	if result is None:
		logger('Simkl scrobble pause', 'no response for media_id=%s percent=%s' % (media_id, percent))
	# Local echo is authoritative for immediate resume UI.
	try: _echo_pause(result, media, media_id, percent, season, episode, title=title)
	except Exception as e: logger('Simkl progress echo', str(e))
	# Full activity sync on every pause is expensive and usually throttled to a no-op.
	# SimklMonitor will pick up remote playback on its interval.
	if refresh_tracker:
		try: simkl_sync_activities(bypass_throttle=True)
		except: pass
...
def simkl_calendar_days(recently_aired, current_date):
	...
	previous_days = int(get_setting('morbius.simkl.calendar_previous_days', '0'))
	future_days = int(get_setting('morbius.simkl.calendar_future_days', '7'))
