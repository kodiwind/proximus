...
class Movies:
	...
	def fetch_list(self):
		...
		try:
			...
			if page_no == 1 and not self.is_external: kodi_utils.set_property('morbius.exit_params', kodi_utils.folder_path())
	...
	def build_movie_content(self, _position, _id):
		try:
			...
			set_properties({
				...
				'morbius.extras_params': extras_params,
				'morbius.options_params': options_params,
				'morbius.playback_options_params': playback_options_params,
				'morbius.browse_movie_set_params': browse_movie_set_params,
				'morbius.browse_recommended_params': browse_recommended_params,
				'morbius.browse_more_like_this_params': browse_more_like_this_params,
				'morbius.browse_similar_params': browse_similar_params,
				'morbius.browse_in_trakt_list_params': browse_in_trakt_list_params,
				'morbius.trakt_manager_params': trakt_manager_params,
				'morbius.personal_manager_params': personal_manager_params,
				'morbius.tmdb_manager_params': tmdb_manager_params,
				'morbius.favorites_manager_params': favorites_manager_params
				})
	...
	def worker(self):
		...
		self.watched_title = 'Trakt' if self.watched_indicators == 1 else 'Simkl' if self.watched_indicators == 2 else 'MDBList' if self.watched_indicators == 3 else 'Morbius'
