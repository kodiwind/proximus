...
class Navigator:
	def __init__(self, params):
		...
		self.category_name = self.params_get('name', 'morbius')
		...
		def _process():
			for count, item in enumerate(browse_list):
				try:
					...
					if icon.startswith('http') or 'plugin.video.morbius' in icon or icon.endswith(('.png', '.jpg', '.jpeg')):
	...
	def my_content(self):
		...
		if s.mdblist_user_active(): self.add({'mode': 'mdblist.list.get_mdblist_lists'}, 'MDBList Lists', 'mdb')
	...
	def tools(self):
		...
		if get_setting('morbius.external_scraper.module') not in ('empty_setting', ''):
		...
		if get_setting('morbius.use_viewtypes', 'true') == 'true' and not get_setting('morbius.manual_viewtypes', 'false') == 'true':
	...
	def changelog_utils(self):
		...
		morbius_clogpath = k.translate_path('special://home/addons/plugin.video.morbius/resources/text/changelog.txt')
		morbius_log_loc = k.translate_path('special://logpath/morbius.log')
		self.add({'mode': 'show_text', 'heading': 'Changelog', 'file': morbius_clogpath, 'font_size': 'large', 'isFolder': 'false'}, 'Changelog', 'lists')
		...
		if get_setting('morbius.addon_debug') == 'true':
			self.add({'mode': 'show_text', 'heading': 'Morbius Debug Log', 'file': morbius_log_loc, 'kodi_log': 'true', 'isFolder': 'false'}, '[COLOR lime]Morbius Debug Log Viewer[/COLOR]', 'lists')
			self.add({'mode': 'clear_morbius_log', 'isFolder': 'false'}, '[COLOR lime]Clear Morbius Debug Log[/COLOR]', 'settings')
		self.end_directory()
	...
	def build_shortcut_folder_contents(self):
		...
		if contents:
			for count, item in enumerate(contents):
				...
				icon = iconImage
				if iconImage:
					if iconImage.startswith('http') or 'plugin.video.morbius' in iconImage: original_image = True
	...
	def exit_media_menu(self):
		params = k.get_property('morbius.exit_params')
		if params: return k.container_refresh_input(params)

	def tips(self):
		tips_location = 'special://home/addons/plugin.video.morbius/resources/text/tips'
