		...
		for item in episodes_data:
			try:
				...
				if progress and not unaired:
					ws.apply_resume_to_listitem(listitem, info_tag, progress, duration)
	...
	def _get_category_name():
		try:
			cat_name = {'episode.progress': 'In Progress Episodes',
						...
						'episode.next_trakt': 'Next Episodes', 'episode.next_morbius': 'Next Episodes',
	...
	if list_type == 'episode.next':
		...
		if include_unwatched != 0:
			if include_unwatched in (1, 3):
				...
				try:
					watchlist = tracking.watchlist_shows()
		...
		calendar_provider = params.get('calendar_provider') or None
		data = tracking.get_my_calendar(recently_aired, get_datetime(), calendar_provider=calendar_provider)
		hidden_list = ws.get_hidden_progress_items(watched_indicators)
		if hidden_list: data = [i for i in data if not i['media_ids']['tmdb'] in hidden_list]
...
def _simkl_nextep_sync():
	"""Background Simkl sync when next-episodes is opened under Simkl XOR. Currently unused; kept for optional soak hooks."""
	def _run():
		try:
