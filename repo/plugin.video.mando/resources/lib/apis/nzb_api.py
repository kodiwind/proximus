...
Settings (per slot 1-3, all under the mando prefix):
...
class NzbIndexerAPI:
	...
	def _search(self, query, cat=DEFAULT_CATS):
		...
		try:
			...
			logger('Mando', 'NZB indexer %s search error: %s' % (self.slot, e))
			return []
...
def search_all(query, cat=DEFAULT_CATS, expiration=2):
	...
	if get_setting('mando.provider.nzb', 'false') != 'true': return []
	seen, merged = set(), []
