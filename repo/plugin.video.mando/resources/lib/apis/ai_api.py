...
def ai_similar_call(media_type, tmdb_id, meta, limit, timeout=30):
	...
	if not model_id:
		logger('Mando', 'AI similar: no available model (all disabled or no API key)')
		return None
	...
	if not result:
		logger('Mando', 'AI similar: %s returned no parseable recommendations' % model_id)
		return None
	return result
