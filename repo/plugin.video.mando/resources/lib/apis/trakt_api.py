...
def trakt_refresh_token():
	...
	kodi_utils.clear_property('mando.trakt_refreshing_token')
...
def trakt_sync_activities(force_update=False):
	...
	def refresh_token_check():
		...
		sync_interval = int(get_setting('mando.trakt.sync_interval', '60')) * 60
		try: expires_at = float(get_setting('mando.trakt.expires'))
		except: expires_at = 0.0
	...
	def _check_daily_expiry():
		return int(time.time()) >= int(get_setting('mando.trakt.next_daily_clear', '0'))
