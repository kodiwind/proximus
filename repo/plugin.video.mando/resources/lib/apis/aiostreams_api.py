...
def _load_profiles_raw():
	try:
		raw = get_setting('mando.%s' % PROFILE_SETTING, '{}') or '{}'
		data = json.loads(raw)
...
def read_active_credentials():
	...
	custom_url = (get_setting('mando.aiostreams.custom_url', '') or '').strip()
...
def instance_id():
	return str(get_setting('mando.aiostreams.instance', '0'))

def base_url():
	current = instance_id()
	if current == CUSTOM_INSTANCE_ID:
		url = get_setting('mando.aiostreams.custom_url', '').strip()
	else:
...
def refresh_base_url_property():
	url = base_url()
	kodi_utils.set_property('mando.aiostreams.base_url', url or '(not set — choose instance or enter Custom URL)')

def sync_instance_display_name():
	label = INSTANCE_LABELS.get(instance_id(), INSTANCE_LABELS['0'])
	kodi_utils.set_property('mando.aiostreams.instance_name', label)
...
def refresh_settings_properties():
	kodi_utils.set_property('mando.aiostreams.available', 'true' if ENABLED else 'false')
