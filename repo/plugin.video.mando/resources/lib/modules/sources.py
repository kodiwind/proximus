...
PROP_NEXTEP_SCRAPE_READY = 'mando.nextep_scrape_ready'
PROP_NEXTEP_SCRAPE_KEY = 'mando.nextep_scrape_key'
PROP_NEXTEP_ALERT_KEY = 'mando.nextep_alert_key'
PROP_NEXTEP_AUTOPLAY_CANCELLED = 'mando.nextep_autoplay_cancelled'
PROP_NEXTEP_NATURAL_END = 'mando.nextep_natural_end'
PROP_AUTOSCRAPE_NEXTEP_READY = 'mando.autoscrape_nextep_ready'
PROP_NEXTEP_HANDOFF_META = 'mando.nextep_handoff_meta'
PROP_NEXTEP_HANDOFF_VISIBLE = 'mando.nextep_handoff.visible'
PROP_RANDOM_CONTINUAL_SKIP_ATTEMPTS = 'mando.random_continual_skip_attempts'
_HANDOFF_COVER = None
...
def schedule_nextep_stashed_play(stash, show_busy=None):
	...
	if nextep_autoplay_cancelled() or nextep_end_play_superseded(stash.get('meta') if stash else None):
		try:
			kodi_utils.logger('Mando', 'Autoplay next episode play: skipped schedule (superseded by user playback)')
		except:
			pass
		return False
	if _nextep_stash_play_in_flight():
		try:
			kodi_utils.logger('Mando', 'Autoplay next episode play: skipped duplicate schedule (stash resolve in flight)')
		except:
			pass
		return False
...
def arm_nextep_handoff_cover(meta=None):
	...
	try:
		...
		if not win or not hasattr(win, 'run'):
			return False
		_HANDOFF_COVER = win
		kodi_utils.logger('Mando', 'Autoscrape handoff cover armed: %s S%02dE%02d' % (
			payload.get('title'), int(payload.get('season') or 0), int(payload.get('episode') or 0)))
		return True
	except Exception as e:
		try:
			kodi_utils.logger('Mando', 'Autoscrape handoff cover arm failed: %s' % e)
		except Exception:
			pass
		return False
	...
	try:
		...
		for _ in range(20):
			if kodi_utils.get_property(PROP_NEXTEP_HANDOFF_VISIBLE) == 'true':
				break
			kodi_utils.sleep(10)
		kodi_utils.logger('Mando', 'Autoscrape handoff cover shown')
		return True
	except Exception as e:
		_HANDOFF_COVER_SHOWN = False
		try:
			kodi_utils.logger('Mando', 'Autoscrape handoff cover show failed: %s' % e)
		except Exception:
			pass
		return False
...
def nextep_handoff_cancelled():
	return nextep_autoplay_cancelled() or kodi_utils.get_property('mando.nextep_prep_declined') == 'true'

def cancel_pending_nextep_on_user_play(meta=None):
	pending = (
		kodi_utils.get_property('mando.nextep_pending') == 'true'
	...
	if pending:
		...
		try:
			kodi_utils.logger('Mando', 'Next episode prep cancelled (user started playback)')
		except:
			pass
...
def cancel_nextep_if_user_target_differs(meta):
	...
	mark_nextep_autoplay_cancelled()
	try:
		kodi_utils.logger('Mando', 'Autoplay next episode: cancelled (user started different title)')
	except:
		pass
	return True
...
class Sources():
	...
	def playback_prep(self, params=None):
		...
		if params_get('nextep_stash_play') == 'true':
			if _nextep_stash_play_in_flight() and not os.path.isfile(_nextep_play_stash_path()):
				try:
					kodi_utils.logger('Mando', 'Autoplay next episode play: skipped duplicate stash resolve (already in flight)')
				except:
					pass
				return
	...
	def filter_results(self, results):
		...
		if self.filter_size_method:
			min_size = string_to_float(get_setting('mando.results.%s_size_min' % self.media_type, '0'), '0') / 1000
		...
		if self.autoplay:
			autoplay_max_mb = string_to_float(get_setting('mando.autoplay.%s_size_max' % self.media_type, '0'), '0')
	...
	def _pref_sort_should_run(self):
		...
		if int(get_setting('mando.filter.sort_to_top', '0')) == 0: return False
	...
	def _process_post_results(self):
		...
		if next_action == 'cache_ignored':
			api_err = kodi_utils.get_property('mando.debrid_cache_api_error')
			if api_err:
	...
	def _finalize_cloud_scraper_properties(self):
		...
		for scraper in self._cloud_scraper_names():
			...
			if thread and thread.is_alive():
				continue
			win_property = kodi_utils.get_property('mando.internal_results.%s' % scraper)
			if win_property in ('checked', '', None):
				kodi_utils.set_property('mando.internal_results.%s' % scraper, 'checked')
	...
	def _process_internal_results(self):
		for i in self.internal_scrapers:
			...
			kodi_utils.set_property('mando.internal_results.%s' % i, 'checked')
			self._sources_quality_count(sources)
	...
	def _clear_properties(self):
		...
		for item in def_internal: kodi_utils.clear_property('mando.internal_results.%s' % item)
		if self.active_folders:
			for item in self.folder_info: kodi_utils.clear_property('mando.internal_results.%s' % item[0])
	...
	def _clear_stale_sources_busy(self):
		...
		if time.time() - busy_at > SOURCES_BUSY_STALE_SECONDS:
			try:
				...
				kodi_utils.logger('Mando', 'Cleared stale sources_busy lock (heartbeat age: %s)' % age)
			except:
				pass
			_clear()
			return True
	...
	def _make_progress_dialog(self):
		...
		for _ in range(40):
			try:
				if kodi_utils.get_property('mando.scrape.ready') == 'true':
					break
			except:
				pass
			kodi_utils.sleep(50)
	...
	def _decline_nextep_prep(self, reason):
		...
		try:
			player = kodi_utils.kodi_player()
			if isinstance(player, MandoPlayer):
				player.decline_nextep_prep(reason)
				return
		except: pass
		kodi_utils.set_property('mando.nextep_prep_declined', 'true')
		kodi_utils.logger('Mando', 'Next episode prep declined: %s' % reason)
	...
	def _autoscrape_playback_ended_naturally(self):
		...
		if natural == 'false':
			return False
		try:
			remaining = kodi_utils.get_property('mando.nextep_remaining')
