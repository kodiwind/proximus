...
def results_format():
	...
	window_format = str(get_setting('mando.results.list_format', 'List'))
...
def store_resolved_to_cloud(debrid_service, pack):
	setting_value = int(get_setting('mando.store_resolved_to_cloud.%s' % debrid_service.lower(), '0'))
	return setting_value in (1, 2) if pack else setting_value == 1

def enabled_debrids_check(debrid_service):
	if not get_setting('mando.%s.enabled' % debrid_service) == 'true': return False
	return authorized_debrid_check(debrid_service)

def authorized_debrid_check(debrid_service):
	if get_setting('mando.%s.token' % debrid_service) in (None, '', 'empty_setting'): return False
	return True
...
def movies_directory():
	return translate_path(get_setting('mando.movies_directory'))
	
def tv_show_directory():
	return translate_path(get_setting('mando.tv_shows_directory'))

def download_directory(media_type):
	download_directories_dict = {'movie': 'mando.movie_download_directory', 'episode': 'mando.tvshow_download_directory', 'thumb_url': 'mando.image_download_directory',
...
def source_folders_directory(media_type, source):
	setting = 'mando.%s.movies_directory' % source if media_type == 'movie' else 'mando.%s.tv_shows_directory' % source
...
def page_limit(is_home):	
	return int(get_setting({True: 'mando.paginate.limit_widgets', False: 'mando.paginate.limit_addon'}[is_home], '20'))
...
def preferred_filters():
	setting = get_setting('mando.filter.preferred_filters')
	if setting in ('empty_setting', ''): return []
	return setting.split(', ')

def include_prerelease_results():
	return int(get_setting('mando.filter.include_prerelease', '0')) == 0
...
def refresh_playback_subs_source():
	...
	current = str(get_setting('mando.playback.subs_source', '0'))
...
def opensubs_username():
	value = get_setting('mando.playback.opensubs_username', 'empty_setting')
	return '' if value in (None, '', '0', 'empty_setting') else str(value).strip()

def opensubs_password():
	value = get_setting('mando.playback.opensubs_password', 'empty_setting')
	return '' if value in (None, '', '0', 'empty_setting') else str(value).strip()
...
def refresh_alert_timing_settings():
	...
	for setting_id in ('stinger_alert.alert_timing', 'autoplay_alert_timing', 'autoscrape_alert_timing'):
		...
		current = str(get_setting('mando.%s' % setting_id, '1'))
		if current not in opts:
...
def subs_language_is_forced_local():
	return str(get_setting('mando.playback.submaker_language', '0')) == FORCED_LOCAL_SUBS_LANGUAGE_INDEX
...
def autoplay_next_episode():
	if auto_play('episode') and get_setting('mando.autoplay_next_episode', 'false') == 'true': return True
	else: return False
...
def autoscrape_next_episode():
	if not auto_play('episode') and get_setting('mando.autoscrape_next_episode', 'false') == 'true': return True
	else: return False
...
def autoscrape_confirm():
	return get_setting('mando.autoscrape_confirm', 'false') == 'true'

def autoplay_prescrape(scrape_provider):
	return get_setting('mando.autoplay.%s' % scrape_provider, 'false') == 'true'
...
def auto_nextep_settings(play_type):
	...
	if play_type == 'autoplay':
		alert_method = int(get_setting('mando.autoplay_alert_method', '0'))
...
def limit_number_quality():
	return int(get_setting('mando.results.limit_number_quality', '0'))

def limit_number_total():
	return int(get_setting('mando.results.limit_number_total', '0'))

def trakt_sync_interval():
	setting = get_setting('mando.trakt.sync_interval', '60')
	interval = int(setting) * 60
	return setting, interval

def lists_sort_order(setting):
	return int(get_setting('mando.sort.%s' % setting, '0'))

def personal_lists_sort_unseen_to_top():
	return get_setting('mando.personal_list.sort_unseen_to_top') == 'true'

def personal_lists_unseen_highlight():
	if get_setting('mando.personal_list.highlight_unseen', 'false') == 'false': return None
	return get_setting('mando.personal_list.unseen_highlight', 'FF4DDBFF')

def personal_lists_show_author():
	return get_setting('mando.personal_list.show_author', 'true') == 'true'

def show_specials():
	return get_setting('mando.show_specials', 'false') == 'true'

def exclude_specials_from_progress():
	return get_setting('mando.exclude_specials_progress', 'true') == 'true'
...
def calendar_display_format(is_external):
	if is_external:
		...
		setting, default = 'mando.trakt.calendar_display', '0'
	return int(get_setting(setting, default))

def easynews_active():
	if get_setting('mando.provider.easynews', 'false') == 'true': easynews_status = easynews_authorized()
	else: easynews_status = False
	return easynews_status

def easynews_playback_method(query):
	method = int(get_setting('mando.easynews.playback_method', '0'))
	queries = {'retry': lambda: method in (1, 3), 'non_seek': lambda: method in (2, 3),
				'direct_play': lambda: method in (2, 3) and get_setting('mando.easynews.playback_method_limited', 'false') != 'true'}
	setting = queries[query]()
	return setting

def easynews_playback_method_retries():
	return int(get_setting('mando.easynews.playback_method_retries', '1')) + 1

def easynews_authorized():
	easynews_user = get_setting('mando.easynews_user', 'empty_setting')
	easynews_password = get_setting('mando.easynews_password', 'empty_setting')
...
def nzb_indexer_active():
	...
	if get_setting('mando.provider.nzb', 'false') != 'true': return False
	for slot in (1, 2, 3):
		if get_setting('mando.nzb%d.enabled' % slot, 'false') != 'true': continue
		url = get_setting('mando.nzb%d.url' % slot, '')
		key = get_setting('mando.nzb%d.key' % slot, '')
...
def nzb_fallback_search():
	return get_setting('mando.nzb.fallback_search', 'true') == 'true'
...
def aiostreams_preserve_order():
	return get_setting('mando.aiostreams.preserve_order', 'true') == 'true'

def extras_enable_extra_ratings():
	return get_setting('mando.extras.enable_extra_ratings', 'true') == 'true'

def extras_enabled_ratings():
	return get_setting('mando.extras.enabled_ratings', 'Meta, Tom/Critic, Tom/User, IMDb, TMDb').split(', ')

def extras_enable_item_ratings():
	return get_setting('mando.extras.enable_item_ratings', 'false') =='true'

def extras_enable_scrollbars():
	return get_setting('mando.extras.enable_scrollbars', 'false')

def extras_enabled():
	setting = get_setting('mando.extras.enabled', '2000,2050,2051,2052,2053,2054,2055,2056,2057,2058,2059,2060,2061,2062')
...
def check_prescrape_sources(scraper, media_type):
	...
	if scraper == 'folders':
		return get_setting('mando.check.folders') == 'true'
...
def easynews_lang_include_unknown():
	return get_setting('mando.easynews.lang_include_unknown', 'true') == 'true'

def easynews_fallback_search():
	return get_setting('mando.easynews.fallback_search', 'true') == 'true'

def easynews_search_width():
	return int(get_setting('mando.easynews.search_width', '0'))

def size_sort_weighted():
	return get_setting('mando.results.size_sort_weighted', 'false') == 'true'

def results_sort_order():
	sort_direction = -1 if get_setting('mando.results.size_sort_direction') == '0' else 1
	return (
			...
			)[int(get_setting('mando.results.sort_order', '1'))]
...
def provider_sort_ranks():
	...
	nzb_priority = int(get_setting('mando.nzb.priority', '7'))
...
def auto_resume(media_type, autoplay_status):
	return {0: False, 1: True, 2: autoplay_status}[int(get_setting('mando.auto_resume_%s' % media_type))]

def scraping_settings():
	highlight_type = int(get_setting('mando.highlight.type', '0'))
	if highlight_type == 2:
	...
	if highlight_type == 0:
		...
		nzb_highlight = get_setting('mando.provider.nzb_highlight', 'FFD4A017')
		...
		highlight_1080P = get_setting('mando.scraper_1080p_highlight', 'FFE6B800')
...
def mpaa_region():
	return get_setting('mando.mpaa_region', 'US')
...
def media_open_action_skip_inprogress_movie():
	return get_setting('mando.media_open_action_skip_inprogress_movie', 'false') == 'true'

def media_open_action_skip_inprogress_tvshow():
	return get_setting('mando.media_open_action_skip_inprogress_tvshow', 'false') == 'true'
...
def provider_sync_refresh_widgets(provider_index):
	...
	if not key:
		return False
	return get_setting('mando.%s' % key, 'false') == 'true'
...
def nextep_method():
	return int(get_setting('mando.nextep.method', '0'))

def nextep_limit_history():
	return get_setting('mando.nextep.limit_history', 'false') == 'true'

def nextep_limit():
	return int(get_setting('mando.nextep.limit', '20'))

def nextep_include_unwatched():
	return int(get_setting('mando.nextep.include_unwatched', '0'))

def nextep_include_airdate():
	return get_setting('mando.nextep.include_airdate', 'false') == 'true'

def nextep_airing_today():
	return get_setting('mando.nextep.airing_today', 'false') == 'true'
...
def migrate_mdblist_context_menu_for_upgrade(had_existing_settings):
	if get_setting('mando.mdblist.cm_menu_migrated', 'false') == 'true': return False
...
def use_season_name():
	return get_setting('mando.use_season_name', 'false') == 'true'
