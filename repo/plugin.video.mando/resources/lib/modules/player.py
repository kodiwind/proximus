...
PROP_NEXTEP_PREP_SCHEDULED = 'mando.nextep_prep_scheduled'
PROP_NEXTEP_PREP_DECLINED = 'mando.nextep_prep_declined'
...
PROP_ACTIVE_PLAYBACK_KEY = 'mando.active_playback_key'
...
class MandoPlayer(xbmc.Player):
	...
	def make_listing(self):
		...
		if self.is_generic:
			...
			play_name = ku.get_property('mando.tb.play_filename') or self.url
			info_tag.setFilenameAndPath(play_name)
			info_tag.setTitle(os.path.basename(play_name) if play_name else '')
			mime = ku.get_property('mando.tb.play_mime')
			if not mime:
	...
	def _should_prep_next_ep(self):
		...
		if ku.get_property(PROP_NEXTEP_PREP_SCHEDULED) == 'true':
			return False
		if self.autoplay_nextep and ku.get_property('mando.nextep_scrape_ready') == 'true':
			try:
	...
	def _maybe_log_nextep_alert_pending(self):
		...
		if ku.get_property('mando.nextep_scrape_ready') != 'true': return
		try:
			from modules.sources import peek_nextep_autoplay_stash
	...
	def _should_show_autoplay_nextep_alert(self):
		...
		try:
			...
			pass
		if ku.get_property('mando.nextep_scrape_ready') != 'true': return False
		try:
			from modules.sources import peek_nextep_autoplay_stash
