		...
		if data:
			if shuffle_lists:
				...
				if returning_to_list:
					try: data = json.loads(kodi_utils.get_property('mando.trakt.lists.order'))
					except: pass
				else:
					shuffle(data)
					kodi_utils.set_property('mando.trakt.lists.order', json.dumps(data))
			else:
				kodi_utils.clear_property('mando.trakt.lists.order')
