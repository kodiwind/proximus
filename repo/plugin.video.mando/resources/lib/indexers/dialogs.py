...
def window_theme_choice(params):
	if params['type'] == 'theme':
		...
		window_theme_opacity = get_setting('mando.window_theme_opacity', 'CC')
		...
		window_theme = get_setting('mando.window_theme', 'FF1F2020')[2:]
		window_theme_contrast = get_setting('mando.window_theme_contrast', 'FF4a4347')
...
def addon_icon_choice(params):
	...
	icons_path = 'special://home/addons/plugin.video.mando/resources/media/addon_icons'
	...
	kodi_utils.set_property('mando.addon_icon', icon_path)
	kodi_utils.set_property('mando.addon_icon_mini', os.path.join(kodi_utils.addon_info('path'), 'resources', 'media', 'addon_icons', 'minis', new_icon))
...
def set_quality_choice(params):
	...
	fl = ['4K', '1080p', '720p', 'SD']
	q_setting = get_setting('mando.%s' % quality_setting).split(', ')
	...
	if not orig_button_dict:
		for _type in ('movie', 'tvshow'):
			...
			for item in range(10, 18):
				...
				try:
					button_action = get_setting('mando.%s' % setting_id)
					button_label = extras_button_label_values[_type][button_action]
				except:
					set_setting(setting_id.replace('mando.', ''), default_setting_values(setting_id)['setting_default'])
					button_action = get_setting('mando.%s' % setting_id)
					button_label = extras_button_label_values[_type][button_action]
...
def extras_ratings_choice(params={}):
	...
	list_items = [{'line1': i[0], 'icon': 'mando_flags/ratings/%s' % i[2]} for i in choices]
...
def set_language_filter_choice(params):
	...
	set_filter = get_setting('mando.%s' % filter_setting_id).split(', ')
...
def scraper_color_choice(params):
	...
	current_setting, original_highlight = get_setting('mando.%s' % setting), default_setting_values(setting)['setting_default']
...
def sort_default_choice(params):
	...
	current = list_sort.parse_spec(get_setting('mando.%s' % setting_id, ''))
