...
class Navigator:
	...
	def tools(self):
		...
		if get_setting('mando.use_viewtypes', 'true') == 'true' and not get_setting('mando.manual_viewtypes', 'false') == 'true':
	...
	def changelog_utils(self):
		...
		mando_clogpath = k.translate_path('special://home/addons/plugin.video.mando/resources/text/changelog.txt')
		self.add({'mode': 'show_text', 'heading': 'Changelog', 'file': mando_clogpath, 'font_size': 'large', 'isFolder': 'false'}, 'Changelog', 'lists')
	...
	def exit_media_menu(self):
		params = k.get_property('mando.exit_params')
	...
	def _set_submenu_exit_params(self, menu_type=None):
		...
		if menu_type in parent:
			k.set_property('mando.exit_params', k.build_folder_url({'mode': 'navigator.main', 'action': parent[menu_type]}))
		else:
			k.set_property('mando.exit_params', k.build_folder_url({'mode': 'navigator.main', 'action': 'RootList'}))

	def _set_exit_params(self, parent_params):
		if self.is_external: return
		k.set_property('mando.exit_params', k.build_folder_url(parent_params))

	def tips(self):
		tips_location = 'special://home/addons/plugin.video.mando/resources/text/tips'
