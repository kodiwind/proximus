...
def _set_mdbl_browse_exit_params():
	...
	kodi_utils.set_property('mando.exit_params', _mdbl_parent_exit_path())
...
def _set_mdbl_list_exit_params(params):
	...
	if page_no != 1: return
	kodi_utils.set_property('mando.exit_params', _mdbl_parent_exit_path(params))
...
def get_mdbl_lists(params):
	...
	if params.get('shuffle', 'false') == 'true':
		lists = _mdbl_shuffle_catalogue(lists, 'mando.mdblist.my_lists.lists.order')
	else:
...
def get_mdbl_liked_lists(params):
	...
	if params.get('shuffle', 'false') == 'true':
		lists = _mdbl_shuffle_catalogue(lists, 'mando.mdblist.liked_lists.lists.order')
	else:
...
def get_mdbl_top_lists(params):
	...
	if params.get('shuffle', 'false') == 'true':
		lists = _mdbl_shuffle_catalogue(lists, 'mando.mdblist.user_lists.lists.order')
	kodi_utils.add_items(handle, list(_process()))
	kodi_utils.set_content(handle, kodi_utils.MENU_FOLDER_CONTENT)
